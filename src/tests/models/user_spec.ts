...
describe('User Model', () => {
  ...
  it('can create user', async () => {
    ...
    const userId: number = dbSetup.users.length + 1
  ...
  it('can authenticate valid user', async () => {
    ...
    const jwt = await store.authenticate(
      testUser.username,
      testUser.password
    );
    ...
    if (jwt) {
      ...
      const verifyObj: jsonwebtoken.JwtPayload  = jsonwebtoken.verify(jwt, TOKEN_SECRET as string) as jsonwebtoken.JwtPayload
