...
type MockDataSetType = {
  ...
  admin: User,
  user: User,
  userWithActiveOrder: User,
  categories: Category[],
  products: Product[],
  orders: Order[],
  activeOrder: Order,
  orderItems: OrderItem[],
  numberOfItemsInActiveOrder: number,
  completedOrders: Set<number | undefined>,
  numberOfItemsInCompletedOrders: number  
}
...
const userWithActiveOrder = users.filter(
  (user) =>
...
const numberOfItemsInActiveOrder = orderItems.reduce(
  (total, currentItem) =>
    currentItem.order_id == activeOrder.id ? total + 1 : total,
  0
);
...
const isCompletedByUser = (order: Order) => {
...
const numberOfItemsInCompletedOrders = orderItems.reduce(
  (total, currentItem) =>
    completedOrders.has(currentItem.order_id) ? total + 1 : total,
  0
);
  

const mockDataSet: MockDataSetType = {
  users: users,
  admin: admin,
  user: user,
